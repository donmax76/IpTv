...
/**
 * Альтернативный плеер на базе libmpv (тот же движок что в Vimu).
 * Используется когда ExoPlayer + nextlib не вытягивает поток
 * (например ARB / HEVC каналы на слабом X4 X4).
 *
 * Round A (текущий): минимальный скелет — открывает URL, играет
 * на SurfaceView, выход по BACK. Без EPG / списка каналов / overlay.
 * Этого достаточно чтобы проверить что libmpv реально подхватывает
 * проблемные каналы.
 *
 * Дальнейшие раунды: добавить overlay со списком, EPG, переключение
 * каналов, gesture-control как в PlayerActivity.
 */
class MpvPlayerActivity : BaseActivity(), SurfaceHolder.Callback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Ландшафт + полный экран как в PlayerActivity.
        window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON)
        // Минимальная разметка: SurfaceView на весь экран + label
        // с именем канала + индикатор загрузки. Полноценный UI
        // (overlay, EPG) добавим в следующем раунде.
    ...
    private fun initMpv() {
        try {
            // Конфигурационная директория mpv в внутреннем хранилище.
            // libmpv записывает туда лог + watch_later state.
            val configDir = filesDir.absolutePath
            MPVLib.create(this, "v")  // 'v' = verbose, для диагностики
            MPVLib.setOptionString("config", "yes")
            MPVLib.setOptionString("config-dir", configDir)
            // Hardware decoding если возможно. На X4 X4 H.265 hw кривой,
            // но поток без HEVC всё равно ускорится. Если глюк — выключим
            // через "no".
