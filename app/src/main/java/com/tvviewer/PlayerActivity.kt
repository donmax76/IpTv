    ...
    private fun createMediaSourceFor(url: String): androidx.media3.exoplayer.source.MediaSource {
        ...
        return when {
            ...
            url.startsWith("rtsp", true) ->
                ...
                // Round 205: вернули setAllowChunklessPreparation(false).
                // В Round 191 я выставил true ради мгновенного
                // переключения, но юзер на билде 282 пожаловался "звук
                // пропал" — некоторые HLS-плейлисты держат audio в
                // отдельной rendition, которая обнаруживается только
                // после парсинга первого медиа-чанка. Скорость не
                // должна ломать функциональность; теряем 1-3 сек
