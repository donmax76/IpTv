...
class AudioEqualizer(private val sampleRate: Int = 48000) {
    ...
    var trebleGainDb: Float = 0f
        set(value) { field = value.coerceIn(-10f, 10f); computeTrebleCoeffs() }

    init { computeBassCoeffs(); computeTrebleCoeffs() }

    private fun computeBassCoeffs() {
        computeShelfCoeffs(300f, bassGainDb, true).let { (b0, b1, b2, a1, a2) ->
            bassB0 = b0; bassB1 = b1; bassB2 = b2; bassA1 = a1; bassA2 = a2
        }
    }

    private fun computeTrebleCoeffs() {
        computeShelfCoeffs(3000f, trebleGainDb, false).let { (b0, b1, b2, a1, a2) ->
    ...
    private fun computeShelfCoeffs(freq: Float, gainDb: Float, isLowShelf: Boolean): FloatArray {
        if (abs(gainDb) < 0.1f) return floatArrayOf(1f, 0f, 0f, 0f, 0f)
        val A = 10f.pow(gainDb / 40f)
        val w0 = 2f * PI.toFloat() * freq / sampleRate
        val cosW0 = cos(w0); val sinW0 = sin(w0)
        val alpha = sinW0 / 2f * sqrt((A + 1f / A) * (1f / 1f - 1f) + 2f)
